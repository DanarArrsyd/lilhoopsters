        ...
        <article class="bg-surface border border-line rounded-2xl overflow-hidden">
            ...
            <div class="p-5">
                <div class="flex items-center gap-2 mb-1.5">
                    @if ($post->is_pinned)
                        <span class="inline-flex items-center gap-1 text-3xs font-bold uppercase tracking-wide text-[#B45309] bg-[#B45309]/10 rounded-md px-1.5 py-0.5">
                            ...
                            {{ __('messages.news.pinned') }}
                        </span>
                    @endif
                    <span class="text-2xs text-faint">{{ $post->published_at?->format('d M Y') }}</span>
                </div>
                ...
                @if ($post->author)
                    <p class="text-2xs text-faint mt-3">{{ __('messages.news.posted_by', ['name' => $post->author->name]) }}</p>
                @endif
            </div>
        </article>
    @empty
        <x-empty-state :title="__('messages.news.empty_title')" :description="__('messages.news.empty_desc')" />
    @endforelse
