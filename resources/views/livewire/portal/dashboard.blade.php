<div>
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-500">Here's what's happening with your players</p>
    </div>

    {{-- Action Required --}}
    @if ($stats['pending_enrollments'] > 0 || $stats['pending_payments'] > 0 || $stats['pending_children'] > 0)
        <h3 class="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-3">Needs Attention</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">

            @if ($stats['pending_children'] > 0)
                <a href="{{ route('parent.players') }}" class="block">
                    <x-card padding="p-5" class="hover:shadow-md transition-shadow cursor-pointer border-l-4 border-l-amber-400">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-xs font-medium text-slate-500 uppercase tracking-wide">Pending Registration</p>
                                <p class="text-3xl font-bold text-slate-900 mt-1">{{ $stats['pending_children'] }}</p>
                            </div>
                        ...
                        <p class="text-xs text-amber-600 mt-2 font-medium">Waiting for admin approval →</p>
                    </x-card>
                </a>
            @endif
            ...
            @if ($stats['pending_payments'] > 0)
                <a href="{{ route('parent.payments') }}" class="block">
                    <x-card padding="p-5" class="hover:shadow-md transition-shadow cursor-pointer border-l-4 border-l-amber-400">
                        ...
                        <p class="text-xs text-amber-600 mt-2 font-medium">Upload payment proof →</p>
                    </x-card>
                </a>
            @endif
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <x-card padding="p-5" class="hover:shadow-md transition-shadow cursor-pointer">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 mt-1">{{ $stats['active_children'] }}</p>
                    </div>
        ...
        <a href="{{ route('parent.enroll') }}" class="block">
            <x-card padding="p-5" class="hover:shadow-md transition-shadow cursor-pointer">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-medium text-slate-500 uppercase tracking-wide">Active Enrollments</p>
                        <p class="text-3xl font-bold text-slate-900 mt-1">{{ $stats['active_enrollments'] }}</p>
                    </div>
            ...
            <x-card padding="p-5" class="hover:shadow-md transition-shadow cursor-pointer">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-slate-900 mt-1">{{ $stats['paid_payments'] }}</p>
                    </div>
