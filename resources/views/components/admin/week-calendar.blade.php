    ...
    <div class="border-t border-line pt-4">
        ...
        @if ($todaySchedules->isEmpty())
            <div class="text-center py-3">
                ...
                @endphp
                @if ($nextDay)
                    <p class="text-3xs text-navy/60 mt-1">{{ __('messages.admin.dashboard.next') }} <span class="font-semibold">{{ $nextDay }}</span></p>
                @endif
            </div>
        @else
            <div class="space-y-3">
                @foreach ($todaySchedules as $sched)
                    <div class="flex items-start gap-2.5">
                        ...
                        @if ($sched->type === 'private')
                            <span class="text-4xs font-bold uppercase bg-purple-100 text-purple-700 px-1 py-0.5 rounded-full shrink-0">PVT</span>
                        @endif
                    </div>
                @endforeach
            </div>
    ...
    @if ($hasOther)
        <div class="border-t border-line pt-4 mt-4" x-data="{ open: false }">
            <button type="button" @click="open = !open" class="w-full flex items-center justify-between lg:pointer-events-none">
                <p class="text-3xs font-bold uppercase tracking-widest text-faint">{{ __('messages.admin.dashboard.rest_of_week') }}</p>
